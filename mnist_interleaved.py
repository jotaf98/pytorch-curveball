...
def train(args, model, device, train_loader, curveball, sgd, epoch, logger):
  model.train()
  for batch_idx, (data, target) in enumerate(train_loader):
    ...
    if batch_idx % args.interleave == 0:
      (loss, predictions) = curveball.step(model_fn, loss_fn)
    else:
      sgd.zero_grad()
...
def main():
  ...
  parser.add_argument('--loss', choices=['crossentropy', 'mse'], default='crossentropy',
            help='loss function')
  parser.add_argument('--interleave', type=int, default=5)
  ...
  args.outputdir += '/' + args.loss + '/curveball-interleaved/' + args.experiment
  ...
  curveball = CurveBall(model.parameters(), lr=args.lr, momentum=args.momentum, lambd=lambd, auto_lambda=not args.no_auto_lambda)

  # open logging stream
  with Logger(args.outputdir, meta=args) as logger:
    # do training
    for epoch in range(1, args.epochs + 1):
      train(args, model, device, train_loader, curveball, sgd, epoch, logger)
